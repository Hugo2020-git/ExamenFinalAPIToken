...
public class MyStepdefs {
    ...
    @When("^Envio la solicitud (GET) a '(.*)' para obtener un token$")
    public void envioLaSolicitudGETAApiAuthenticationTokenJsonParaObtenerUnToken(String method, String url) throws JSONException {
        ...
        TOKEN = JsonHelper.getValueFromJSON(response.getResponseBody(),"TokenString");
        System.out.println("Body de respuesta: "+this.replaceVariables(response.getResponseBody()));
    }

    @When("^Envio la solicitud (POST|PUT|DELETE|GET) a '(.*)' con el json$")
    public void envioLaSolicitudApiProjectConElJson(String method, String url, String jsonBody) {
        ...
        request.addHeaders(TOKEN_HEADER,TOKEN);
        response = FactoryRequest.make(method.toLowerCase()).send(request);
     }

    @When("^Para usuario envio la solicitud (PUT) a '(.*)' con el json$")
    public void paraUsuarioEnvioLaSolicitudPUTAApiUserJsonConElJson(String method,String url, String jsonBody) {
        method2 = method;
        RequestInformation request = new RequestInformation();
        request.setUrl(HOST+this.replaceVariables(url));
        request.setBody(this.replaceVariables(jsonBody));
    ...
    @When("^Para usuario envio la solicitud (POST) a '(.*)' con el json$")
    public void paraCrearUsuarioEnvioLaSolicitudPOSTAApiUserJsonConElJson(String method, String url,String jsonBody) {
        method2 = method;
        RequestInformation request = new RequestInformation();
        request.setUrl(HOST+this.replaceVariables(url));
        request.setBody(this.replaceVariables(jsonBody));
    ...
    @Then("^Se espera el codigo de respuesta (\\d+)$")
    ...
    @And("^Obtengo el valor de propiedad '(.*)' y se guarda en (.*)$")
